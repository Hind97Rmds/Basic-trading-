...
bot_state: dict = {
    ...
    'strategy_wide_zone': False,       # False = 20/80 zone thresholds, True = widened 25/75
...
STRAT_STOCH_BUY_LEVEL_DEFAULT = 20    # crossover must occur with both lines <= this
STRAT_STOCH_SELL_LEVEL_DEFAULT = 80   # crossover must occur with both lines >= this
STRAT_STOCH_BUY_LEVEL_WIDE = 25       # widened variant (toggle in Telegram)
STRAT_STOCH_SELL_LEVEL_WIDE = 75      # widened variant (toggle in Telegram)
...
def _strategy_zone_levels() -> tuple[float, float]:
    """Returns (buy_level, sell_level) -- widened to 25/75 when
    bot_state['strategy_wide_zone'] is toggled on, default 20/80 otherwise."""
    if bot_state.get('strategy_wide_zone', False):
        return STRAT_STOCH_BUY_LEVEL_WIDE, STRAT_STOCH_SELL_LEVEL_WIDE
    return STRAT_STOCH_BUY_LEVEL_DEFAULT, STRAT_STOCH_SELL_LEVEL_DEFAULT
...
def evaluate_strategy_signal(df: "pd.DataFrame", shift: int) -> dict | None:
    ...
    buy_level, sell_level = _strategy_zone_levels()
    ...
    if ema15 > ema50 > ema150 and cross_up and k_prev <= buy_level and d_prev <= buy_level:
    ...
    if ema150 > ema50 > ema15 and cross_down and k_prev >= sell_level and d_prev >= sell_level:
        return {'is_buy': False, 'ema15': ema15, 'ema50': ema50, 'ema150': ema150, 'k': k, 'd': d,
                'candle_time': df['time'].iloc[idx] if 'time' in df.columns else None}
    return None

def strategy_signal_breakdown(df: "pd.DataFrame", shift: int) -> dict:
    """Diagnostic companion to evaluate_strategy_signal: never gates a
    trade, just reports the actual value of every sub-condition at the
    evaluated candle -- which EMA order is/isn't satisfied, whether a
    cross happened, and how far %K/%D actually were from the zone
    threshold when they didn't qualify. This is what turns
    'no_signal_this_candle' from a dead end into something a human can
    actually use to tell "so close" apart from "not even in the same
    neighborhood"."""
    idx = len(df) - 1 - shift
    prev_idx = idx - 1
    if idx < 1 or prev_idx < 0:
        return {'reason': 'insufficient_history_for_shift'}
    ema15, ema50, ema150 = df['ema15'].iloc[idx], df['ema50'].iloc[idx], df['ema150'].iloc[idx]
    k, d = df['stoch_k'].iloc[idx], df['stoch_d'].iloc[idx]
    k_prev, d_prev = df['stoch_k'].iloc[prev_idx], df['stoch_d'].iloc[prev_idx]
    if any(pd.isna(v) for v in (ema15, ema50, ema150, k, d, k_prev, d_prev)):
        return {'reason': 'nan_values_in_window'}

    buy_level, sell_level = _strategy_zone_levels()
    ema_bull = ema15 > ema50 > ema150
    ema_bear = ema150 > ema50 > ema15
    ema_state = 'bullish' if ema_bull else ('bearish' if ema_bear else 'mixed/no_order')
    cross_up = (k_prev <= d_prev) and (k > d)
    cross_down = (k_prev >= d_prev) and (k < d)
    zone_ok_buy = k_prev <= buy_level and d_prev <= buy_level
    zone_ok_sell = k_prev >= sell_level and d_prev >= sell_level

    missing = []
    if ema_bull:
        if not cross_up: missing.append('no_stoch_cross_up_this_candle')
        elif not zone_ok_buy: missing.append(f'cross_up_but_outside_buy_zone(k_prev={k_prev:.1f},d_prev={d_prev:.1f},need<={buy_level})')
    elif ema_bear:
        if not cross_down: missing.append('no_stoch_cross_down_this_candle')
        elif not zone_ok_sell: missing.append(f'cross_down_but_outside_sell_zone(k_prev={k_prev:.1f},d_prev={d_prev:.1f},need>={sell_level})')
    else:
        missing.append('ema_not_stacked_either_direction')

    return {
        'ema_state': ema_state, 'ema15': round(float(ema15), 3), 'ema50': round(float(ema50), 3), 'ema150': round(float(ema150), 3),
        'stoch_k': round(float(k), 2), 'stoch_d': round(float(d), 2),
        'stoch_k_prev': round(float(k_prev), 2), 'stoch_d_prev': round(float(d_prev), 2),
        'cross_up': bool(cross_up), 'cross_down': bool(cross_down),
        'zone_buy_level': buy_level, 'zone_sell_level': sell_level,
        'missing_condition': '; '.join(missing) if missing else 'none(should_have_fired)',
    }
...
def get_strategy_keyboard() -> dict:
    ...
    wide_zone = bot_state.get('strategy_wide_zone', False)
    zone_lbl = 'موسّعة 25/75' if wide_zone else 'افتراضية 20/80'
    ...
    rows += [
        ...
        [{'text': f'نطاق الستوكاستيك (شراء/بيع): {zone_lbl}', 'callback_data': 'strategy_toggle_zone_width'}],
...
def get_strategy_bt_keyboard() -> dict:
    ...
    return {'inline_keyboard': [
        [{'text': '⏱️ آخر ساعة متاحة', 'callback_data': 'gbt_h1'}],
...
async def strategy_run_diagnostics() -> str:
    ...
    for symbol in active_symbols:
        ...
        for tf in enabled_tfs:
            ...
            if sig is None:
                bd = strategy_signal_breakdown(df, shift)
                if 'missing_condition' in bd:
                    lines.append(f"[{tf}] ❌ الناقص: {bd['missing_condition']}")
                else:
                    lines.append(f"[{tf}] ❌ لا توجد إشارة ({bd.get('reason', 'unknown')}).")
                continue
            ...
            for symbol in active_symbols:
                try:
                    ...
                    for tf in enabled_tfs:
                        ...
                        sig = evaluate_strategy_signal(df, shift)
                        if sig is None:
                            breakdown = strategy_signal_breakdown(df, shift)
                            _diag_log_add({'ts': detect_time, 'symbol': symbol, 'tf': tf,
                                           'skip_reason': 'no_signal_this_candle', **breakdown})
                            continue
    ...
    try:
        ...
        for symbol in active_symbols:
            ...
            for btf in enabled_tfs:
                ...
                buy_level, sell_level = _strategy_zone_levels()
                buy_cond = (ema15 > ema50) & (ema50 > ema150) & cross_up & (k_prev <= buy_level) & (d_prev <= buy_level)
                sell_cond = (ema150 > ema50) & (ema50 > ema15) & cross_down & (k_prev >= sell_level) & (d_prev >= sell_level)
...
async def _handle_callback(d: str, chat_id: int, msg_id: int) -> None:
    ...
    if d == 'menu_main':
        ...
        await save_bot_persistence()
        await _show(chat_id, msg_id, 'إعدادات الاستراتيجية:', get_strategy_keyboard())
    elif d == 'strategy_toggle_zone_width':
        bot_state['strategy_wide_zone'] = not bot_state.get('strategy_wide_zone', False)
        await save_bot_persistence()
        await _show(chat_id, msg_id, 'إعدادات الاستراتيجية:', get_strategy_keyboard())
        ...
        if not bot_state['is_backtesting']:
            bot_state['is_backtesting'] = True
            _safe_task(run_strategy_backtest(start_dt, end_dt), 'backtest_preset_h1')
        await _show(chat_id, msg_id, f'⏳ باكتيست آخر ساعة يعمل...', get_strategy_bt_keyboard())
